    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            message = await ctx.send(
                f"{error.param.name.capitalize()} is a required argument that is missing.", delete_after=10
            )
        elif isinstance(error, commands.NotOwner):
            message = await ctx.send(
                "You do not have the necessary permissions to execute this command.", delete_after=10
            )
            ...
            message = await ctx.send("You cannot use this command.", delete_after=10)
        elif isinstance(error, discord.errors.Forbidden):
            message = await ctx.send(
                "I do not have the necessary permissions to perform this command.", delete_after=10
            )
        elif isinstance(error, commands.CommandOnCooldown):
            message = await ctx.send(
                f"This command is on a {round(error.retry_after, 2)}s cooldown", delete_after=error.retry_after
            )
            ...
            message = await ctx.send(
                ...
                'Error traceback has been sent to the Developer.',
                delete_after=30,
            )
